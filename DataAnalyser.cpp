...
bool DataAnalyser::findEndIndex()
{
    ...
    if (dataLength < endCodeIndex) {
        ...
        if (endCodeIndex + START_END_CODE_LENGTH < this->currDataStr.length()) {
            expectedEndCode = this->currDataStr.substr(endCodeIndex, endCodeIndex + START_END_CODE_LENGTH);
        }
...
void DataAnalyser::analyseDataPayload()
{
    ...
    for (int i = 0; i < dataPayload.length() - DATA_BLOCK_LEN_HEX; i = i + DATA_BLOCK_LEN_HEX) {
        string dataChunk = dataPayload.substr(i, i + DATA_BLOCK_LEN_HEX);
    ...
    int dataPayloadLen = buffLen - DATA_PARAMS_LEN;
    ...
    vector<int> numSeq = Calculator::getNumberSequence(numSeqLen);

    if (indexListInts != numSeq) {
        this->logger->logError("Indices in the list were not sorted! ");
                              // + indexList.toString() + "num seq: " + numSeq.toString() + " " + dataPayload);
    }
}
...
bool DataAnalyser::checkDataIndex()
{
    ...
    if (prevIndex != MAX_DATA_INDEX) {
        if (currIndex != prevIndex + 1) {
            this->logger->logError("Unexpected data index! Prev index: " + to_string(prevIndex) + " " + prevDataItem.dataIndexHex +
                                   " curr data: " +
